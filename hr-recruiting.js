...
async function hrRenderCandidate(mount, id) {
  ...
  body.innerHTML = `
    ...
    ${a.ai_summary ? `<div class="hr-card"><h2>AI summary</h2><div style="white-space:pre-wrap">${hrEsc(a.ai_summary)}</div></div>` : ""}
  ...
  if (screenBtn) {
    screenBtn.addEventListener("click", async () => {
      ...
      st.textContent = "Screening…"; st.className = "hr-status";
      try {
        const r = await hrApi("/api/hr/applicants/" + id + "/screen", { method: "POST" });
        st.textContent = r.message || "Done."; st.className = "hr-status ok";
      } catch (e) { st.textContent = e.message; st.className = "hr-status err"; }
    });
  }
